...
func TestPacketDispatcherChain(t *testing.T) {
	tf := func(t *testing.T, num int, errIndex int) {
		...
		defer ctl.Finish()
		fc := test.NewMockFuncCall(ctl)
		errFailed := errors.New("dispatch failed")
		lists := make([]process.PacketDispatcherFilter, 0, num)
		for k := 0; k < num; k++ {
			index := k
			runFlag := false
			if errIndex >= 0 {
			...
			lists = append(lists, func(data []byte, next process.PacketDispatcherFunc) (err error) {
				if errIndex >= 0 {
					if index == errIndex {
						return errFailed
		...
		f := process.PacketDispatcherChain(lists...)
