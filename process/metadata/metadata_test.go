...
func BenchmarkAppendToOutgoingContext(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		src := ctx
		for i := 0; i < num; i++ {
			src = AppendToOutgoingContext(src, "k1", "v1", "k2", "v2")
		}
	}
}
...
func BenchmarkMDWithInt(b *testing.B) {
	...
	b.Run("set2", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			md.SetInt("a", 1000000, 10000000)
		}
	})
	b.Run("set2", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			md.SetInt("a", 1000000, 10000000, 230239093)
		}
	})
