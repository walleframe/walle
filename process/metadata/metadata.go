...
// NOTE: copy from grpc and add some functions.
...
// Set sets the value of a given key with a int64 values.
//
// k is converted to lowercase before storing in md.
func (md MD) SetInt(k string, vals ...int64) {
	...
	k = strings.ToLower(k)
	if len(vals) == 1 {
		md[k] = []string{strconv.FormatInt(vals[0], 10)}
		return
	}

	md[k] = make([]string, 0, len(vals))
	for _, v := range vals {
		md[k] = append(md[k], strconv.FormatInt(v,10))
	}
}
